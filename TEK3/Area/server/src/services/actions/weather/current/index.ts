...
const presentWeather: {[key: string]: string} = {
...
async function weather(params: any) {
    let latitude = 48.8534;
    let longitude = 2.3488;
    if (params.city) {
        const cityResp = await axios.get(`https://geocoding-api.open-meteo.com/v1/search?name=`+ params.city + `&count=1&language=en&format=json`);
    ...
    const resp = await axios.get("https://api.open-meteo.com/v1/forecast?latitude="+latitude+"&longitude="+longitude+"&daily=weathercode&timezone=Europe%2FBerlin&forecast_days=1");
